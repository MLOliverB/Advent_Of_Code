...
max_dist = 0
max_scanners = ""
for scanner_0 in scanners:
    for scanner_1 in scanners:
        if scanner_0.label != scanner_1.label:
            dist = int(np.linalg.norm(scanner_0.absolute_zero - scanner_1.absolute_zero, ord=1))
            if dist > max_dist:
                max_dist = dist
                max_scanners = scanner_0.label + ", " + scanner_1.label
print("Max distance between scanners ({}) being {}".format(max_scanners, max_dist))
