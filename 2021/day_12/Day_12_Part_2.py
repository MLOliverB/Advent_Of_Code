        ...
        for i in range(cc_len-1, -1, -1):
            if connected_caves[i] == start:
                ...
                if connected_caves[i] in path:
                    ...
                    # Check if we have visited it only once
                    count = 0
                    for cave in path:
                        if cave == connected_caves[i]:
                            count += 1
                    if count > 1:
                        # If we already visited twice, pop
                        include_ix.pop(i)
                    else:
                        # Check if we have already visited another small cave twice
                        small_caves = {}
                        # Count occurences of each small cave
                        for cave in path:
                            if not cave.is_large:
                                if cave not in small_caves:
                                    ...
                                    small_caves[cave] += 1
                        # If a single small cave appears twice, we can't visit any other small caves twice
                        for cave in small_caves:
                            if small_caves[cave] > 1:
                                include_ix.pop(i)
                                break
        if len(include_ix) > 0:
