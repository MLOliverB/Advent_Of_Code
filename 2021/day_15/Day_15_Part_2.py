...
tile_0_matrix = np.full((len(input_matrix), len(input_matrix[0])), None, dtype=object)
# Initialize the vertices
for i in range(tile_0_matrix.shape[0]):
    for j in range(tile_0_matrix.shape[1]):
...
vertex_matrix = np.full((rows*row_tiles, cols*col_tiles), None, dtype=object)
# Do the first tile
for i in range(rows):
    for j in range(cols):
        v = Vertex(input_matrix[i][j], i, j)
...
for i in range(rows):
    for j in range(cols):
        for col_tile in range(1, col_tiles):
            weight = vertex_matrix[i, ((col_tile-1)*cols)+j].weight + 1
            if weight > 9:
                weight = 1
            vertex_matrix[i, ((col_tile)*cols)+j] = Vertex(weight, i, ((col_tile)*cols)+j)
# Pull it down through all columns
for i in range(rows):
    for j in range(cols*col_tiles):
        for row_tile in range(1, row_tiles):
            weight = vertex_matrix[((row_tile-1)*rows)+i, j].weight + 1
            if weight > 9:
                weight = 1
            vertex_matrix[((row_tile)*rows)+i, j] = Vertex(weight, ((row_tile)*rows)+i, j)
